...
class StarReLU(nn.Module):
    ...
    def _init_(self, scale_value=1.0, bias_value=0.0,
...
class KernelSpatialModulation_Global(nn.Module):
    def _init_(self, in_planes, out_planes, kernel_size, groups=1, reduction=0.0625, kernel_num=4, min_channel=16, 
                 temp=1.0, kernel_temp=None, kernel_att_init='dyconv_as_extra', 
                 att_multi=1.0,  # 🔥 সমাধান: ডিফল্ট মান 2.0 থেকে 1.0 করা হয়েছে
                 ksm_only_kernel_att=False, att_grid=1, stride=1, spatial_freq_decompose=False,
                 act_type='sigmoid'):
        super(KernelSpatialModulation_Global, self)._init_()
        ...
        self.relu = StarReLU() # দ্রষ্টব্য: অস্থিতিশীলতা থাকলে এটিকে nn.ReLU(inplace=True) দিয়ে পরিবর্তন করতে পারেন
...
class KernelSpatialModulation_Local(nn.Module):
    ...
    """
    def _init_(self, channel=None, kernel_num=1, out_n=1, k_size=3, use_global=False):
        super(KernelSpatialModulation_Local, self)._init_()
        self.kn = kernel_num
        self.out_n = out_n
    ...
    def forward(self, x):
        ...
        if self.use_ksm_local:
            ...
            if self.ksm_local_act == 'sigmoid':
                hr_att = hr_att_logit.sigmoid() * self.att_multi # 🔥 এখানে att_multi=1.0 ব্যবহৃত হবে
